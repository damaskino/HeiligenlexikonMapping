...
hundred_saints_cursor.executescript(
    """DROP TABLE IF EXISTS hundred_saints; CREATE TABLE hundred_saints (id varchar(20), data json)"""
)
...
for idx, (id, content) in enumerate(saints):
    ...
    # print(id)
    # print(content)
    hundred_saints_cursor.execute(
        "insert into hundred_saints values (?, ?)", [id, content]
    )
