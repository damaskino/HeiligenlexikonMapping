...
def get_metric_components_from_result_file(filepath:str):
    with open(current_path, encoding='utf-8') as current_file:
        ...
        true_positives, true_negatives, false_positives, false_negatives = calculate_metric_components_of_results(result_list=results)
        return true_positives, true_negatives, false_positives, false_negatives

def write_metric_component_entries_to_str(metric_component_list: str):
    result_string = ""
    for entry_tuple in metric_component_list:
        result_string+=";".join([entry_tuple[0],entry_tuple[1],entry_tuple[2]])+"\n"
    return result_string

for result_file in os.listdir(matching_results_path):
    current_path = os.path.join(matching_results_path, result_file)

    true_positives, true_negatives, false_positives, false_negatives = get_metric_components_from_result_file(current_path)
    ...
    accuracy, precision, recall = calculate_metrics(tp=tp_count, fp=fp_count, tn=tn_count, fn=fn_count)
    print(f"Accuracy of file: {result_file}: Acc: {accuracy}, P: {precision}, R: {recall} ")
    ...
    with open(eval_file_path, encoding='utf-8', mode="w") as output_file:
        output_string = "File;Accuracy;Precision;Recall\n"+\
